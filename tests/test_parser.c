...
static void test_parser_skips_ipv6_extension_headers_to_udp_payload(void) {
    const unsigned char packet[] = {
        0x60, 0x00, 0x00, 0x00, 0x00, 0x12, 0x3c, 0x40, 0x20, 0x01, 0x0d, 0xb8, 0x00, 0x00, 0x00,
        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01, 0x20, 0x01, 0x0d, 0xb8, 0x00, 0x00,
        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x02, 0x11, 0x00, 0x00, 0x00, 0x00,
        0x00, 0x00, 0x00, 0x04, 0xd2, 0x00, 0x35, 0x00, 0x0a, 0x00, 0x00, 'O',  'K'};
    PacketInfo info;
...
static void test_parser_conservatively_skips_ipv6_fragment_transport(void) {
    const unsigned char packet[] = {
        0x60, 0x00, 0x00, 0x00, 0x00, 0x1c, 0x2c, 0x40, 0x20, 0x01, 0x0d, 0xb8, 0x00, 0x00,
        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01, 0x20, 0x01, 0x0d, 0xb8,
        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x02, 0x06, 0x00,
        0x00, 0x01, 0x00, 0x00, 0x00, 0x01, 0x13, 0x88, 0x01, 0xbb, 0x00, 0x00, 0x00, 0x01,
...
static void test_parser_extracts_icmpv6_summary_and_payload(void) {
    const unsigned char packet[] = {0x60, 0x00, 0x00, 0x00, 0x00, 0x0a, 0x3a, 0x40, 0x20, 0x01,
                                    0x0d, 0xb8, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                                    0x00, 0x00, 0x00, 0x01, 0x20, 0x01, 0x0d, 0xb8, 0x00, 0x00,
                                    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x02,
                                    0x80, 0x00, 0x00, 0x00, 0x00, 0x01, 0x00, 0x01, 'O',  'K'};
    PacketInfo info;

    assert(parser_parse_packet_with_datalink(packet, sizeof(packet), DLT_RAW, &info) == 0);
    assert(info.protocol == PROTO_ICMP);
    assert(info.ip_version == 6);
    assert(info.has_icmp == 1);
    assert(info.icmp_type == 128);
...
static void test_parser_parse_packet_parses_arp_request(void) {
    const unsigned char packet[] = {/* Ethernet header: dst mac, src mac, ethertype=ARP */
                                    0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0x00, 0x11, 0x22, 0x33,
                                    0x44, 0x55, 0x08, 0x06,
...
static void test_parser_parse_packet_parses_arp_reply(void) {
    const unsigned char packet[] = {
        0x00, 0x11, 0x22, 0x33, 0x44, 0x55, 0xaa, 0xbb, 0xcc, 0xdd, 0xee, 0xff, 0x08, 0x06,
        0x00, 0x01, 0x08, 0x00, 0x06, 0x04, 0x00, 0x02, 0xaa, 0xbb, 0xcc, 0xdd, 0xee, 0xff,
        0xc0, 0xa8, 0x01, 0x01, 0x00, 0x11, 0x22, 0x33, 0x44, 0x55, 0xc0, 0xa8, 0x01, 0x0a};
    PacketInfo info;
...
int main(void) {
    ...
    test_parser_skips_ipv6_extension_headers_to_udp_payload();
    test_parser_conservatively_skips_ipv6_fragment_transport();
