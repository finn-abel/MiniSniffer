...
static AppDecodeResult feed_segment(FlowInfo *flow, FlowDirection direction, PacketInfo *packet,
    ...
    assert(tcp_reassembly_process_segment(tcp, packet->tcp_sequence, packet->tcp_flags,
                                          packet->payload,
                                          packet->payload_decode_length) != TCP_REASSEMBLY_DROPPED);
    result = app_decode_stream(packet, stream_buffer_data(&tcp->stream),
                               stream_buffer_length(&tcp->stream), &flow->app);
...
static void test_http_request_split_across_two_tcp_segments(void) {
    ...
    assert(feed_segment(flow, direction, &packet, 100 + sizeof(first) - 1, second,
                        sizeof(second) - 1) == APP_DECODE_OK);
    assert(flow->app_classified);
...
static void test_http_headers_split_across_multiple_packets(void) {
    ...
    assert(feed_segment(flow, direction, &packet, 1 + sizeof(a) - 1, b, sizeof(b) - 1) ==
           APP_DECODE_NEED_MORE);
    assert(feed_segment(flow, direction, &packet, 1 + sizeof(a) - 1 + sizeof(b) - 1, c,
...
static void test_tls_client_hello_split_across_two_segments(void) {
    ...
    assert(feed_segment(flow, direction, &packet, 50, TLS_CLIENT_HELLO_SNI_ALPN, split) ==
           APP_DECODE_NEED_MORE);
    assert(feed_segment(flow, direction, &packet, 50 + (uint32_t)split,
...
static void test_dns_over_tcp_split_across_two_segments(void) {
    ...
    assert(feed_segment(flow, direction, &packet, 10 + (uint32_t)first_len, frame + first_len,
...
static void test_max_stream_buffer_cap_prevents_classification(void) {
    ...
    assert(tcp_reassembly_process_segment(&flow->directions[direction].tcp, packet.tcp_sequence, 0,
                                          packet.payload,
                                          packet.payload_decode_length) == TCP_REASSEMBLY_DROPPED);
    assert(!flow->app_classified);
    flow_table_cleanup(&table);
